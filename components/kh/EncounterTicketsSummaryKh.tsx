...
export default async function EncounterTicketsSummaryKh() {
  ...
  const khmerIndividualInclusions = [
    "ការជួបឯកជនជាមួយ Molly & Alex",
    "ជំនួយក្នុងការថតរូបដោយបុគ្គលិកជំនាញ",
    "ការផ្តល់ចំណីដោយមានការណែនាំត្រឹមត្រូវ",
    "ទទួលបានព័ត៌មាន និងចំណេះដឹងអំពីសត្វកាពីបារ៉ា",
    "ទទួលបានវត្ថុអនុស្សាវរីយ៍ពិសេសដោយឥតគិតថ្លៃ",
    "ប្រើប្រាស់អាងហែលទឹក + ទីធ្លាលេងកម្សាន្ត",
  ];

  const khmerFamilyInclusions = [
    "សម្រាប់មនុស្សពេញវ័យ ២ នាក់ + កុមារ ២ នាក់",
    "ការជួបឯកជន ការថតរូប និងផ្តល់អាហារ",
    "ទទួលបានវត្ថុអនុស្សាវរីយ៍ឥតគិតថ្លៃ",
    "ការប្រើប្រាស់អាងហែលទឹក និងកន្លែងក្មេងលេង",
    "កុមារអាយុក្រោម ៣ ឆ្នាំ មិនគិតថ្លៃសំបុត្រជាដាច់ខាត",
  ];

  return (
    <section id="capybara-experience" className="relative w-full py-16 sm:py-24 bg-gradient-to-b from-[#FFF5F7]/80 via-[#FAF7F2] to-[#F0F9FF]/70 text-[#1A2E1C] overflow-hidden border-t border-[#FDA4AF]/20 font-battambang">
            ...
            <div className="relative flex flex-col justify-between rounded-3xl bg-white/80 backdrop-blur-xs border border-[#1B5E20]/15 shadow-md hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 overflow-hidden group h-full">
               ...
               <div className="p-6 sm:p-8 flex-1 flex flex-col">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mt-auto pt-2">
                    <a
                      href={`https://wa.me/${cleanPhone}?text=${encodeURIComponent(`សួស្តី ខ្ញុំចង់កក់សំបុត្រកញ្ចប់ឯកជនតម្លៃ $${data.individualPrice || 10}។`)}`}
                      target="_blank"
                    ...
                    <a
                      href={`${telegramBaseUrl}?text=${encodeURIComponent(`សួស្តី ខ្ញុំចង់កក់សំបុត្រកញ្ចប់ឯកជនតម្លៃ $${data.individualPrice || 10}។`)}`}
                      target="_blank"
                      rel="noopener noreferrer"
          ...
          <ScrollReveal direction="up" staggerIndex={1} delay={0.1}>
            <div className="relative flex flex-col justify-between rounded-3xl bg-white border-2 border-[#FDA4AF] shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 overflow-hidden group h-full">
               ...
               <div className="relative w-full h-48 sm:h-56">
                  ...
                  <h3 className="absolute bottom-4 left-6 text-2xl font-bold text-white leading-snug">កញ្ចប់គ្រួសារ</h3>
               </div>

               <div className="p-6 sm:p-8 flex-1 flex flex-col">
                  <div className="inline-block px-3 py-1 bg-[#FCE7F3] text-[#BE123C] text-xs font-bold rounded-full mb-3 w-fit">
                     សម្រាប់បទពិសោធន៍ជាគ្រួសារ
                  </div>
                  ...
                  {data.familyPaxNote && (
                    <div className="mb-4 inline-flex items-start bg-rose-50 border border-rose-200 text-rose-700 px-3 py-2 rounded-lg">
                       <span className="text-[10px] font-bold uppercase tracking-wide leading-tight">
                         *មនុស្សពេញវ័យ ២ នាក់ + កុមារ ២ នាក់
                       </span>
                    </div>
                  )}
                  ...
                  <div className="flex items-baseline gap-1 mb-6">
                      ...
                      <span className="text-sm text-gray-500 font-medium">/ គ្រួសារ</span>
                  </div>

                  <ul className="space-y-3 mb-8 flex-1">
                     {khmerFamilyInclusions.map((inclusion, idx) => (
                       <li key={idx} className="flex items-start gap-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mt-auto pt-2">
                    <a
                      href={`https://wa.me/${cleanPhone}?text=${encodeURIComponent(`សួស្តី ខ្ញុំចង់កក់សំបុត្រកញ្ចប់គ្រួសារតម្លៃ $${data.familyPrice || 30}។`)}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center justify-center gap-2 py-3.5 px-4 rounded-xl text-sm font-bold bg-[#25D366] text-white hover:bg-[#20ba59] hover:shadow-md active:scale-95 transition-all duration-200"
                    >
                    ...
                    <a
                      href={`${telegramBaseUrl}?text=${encodeURIComponent(`សួស្តី ខ្ញុំចង់កក់សំបុត្រកញ្ចប់គ្រួសារតម្លៃ $${data.familyPrice || 30}។`)}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center justify-center gap-2 py-3.5 px-4 rounded-xl text-sm font-bold bg-[#2AABEE] text-white hover:bg-[#2298d5] hover:shadow-md active:scale-95 transition-all duration-200"
                    >
        ...
        <ScrollReveal direction="up" delay={0.3}>
          <div className="mt-12 p-5 sm:p-6 rounded-3xl bg-white/90 border border-[#1B5E20]/15 shadow-sm flex flex-col sm:flex-row sm:grid sm:grid-cols-3 gap-6 text-left hover:shadow-md transition-shadow duration-300">
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <h4 className="text-sm font-bold text-[#1A2E1C]">ការកំណត់ចំនួនអ្នកទស្សនា</h4>
                <p className="text-xs text-gray-500 mt-0.5">អតិបរមា ៨ នាក់ក្នុងមួយវគ្គ។</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <h4 className="text-sm font-bold text-[#1A2E1C]">ការផ្លាស់ប្តូរពេលវេលា</h4>
                <p className="text-xs text-gray-500 mt-0.5">អាចប្តូរបានមុនពេល ២៤ ម៉ោង។</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <h4 className="text-sm font-bold text-[#1A2E1C]">សីលធម៌នៃការចិញ្ចឹម</h4>
                <p className="text-xs text-gray-500 mt-0.5">សត្វមានសេរីភាពពេញលេញ។</p>
              </div>
            </div>
          </div>
        </ScrollReveal>
