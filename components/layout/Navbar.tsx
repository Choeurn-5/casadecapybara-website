...
export default function Navbar({ settings }: { settings: GlobalSettings }) {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 z-50 w-full text-[#FAF7F2] transition-all duration-500 ${isScrolled
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="hidden xl:flex items-center space-x-1 2xl:space-x-3 flex-1 justify-center">
              ...
              {/* Language Switcher (Moved into Menu) */}
              <button
                onClick={toggleLanguage}
                className="flex items-center gap-2 text-sm 2xl:text-base font-medium text-[#FAF7F2]/90 hover:text-white px-4 py-2 rounded-full hover:bg-white/10 transition-all duration-300"
              >
                {lang === "EN" ? (
                  <><span className="text-lg">🇰🇭</span> Khmer</>
                ) : (
